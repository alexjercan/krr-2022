...
class JunctionTree:
    ...
    """
    def __init__(self, bn: BayesNet, debug=False):
        self.bn = bn
        self.debug = debug
    ...
    def _triangulate(self, h: nx.Graph) -> nx.Graph:
        th, _ = nx.algorithms.complete_to_chordal_graph(h)
        return th

    def _create_clique_graph(self, th: nx.Graph) -> nx.Graph:
        cliques = list(nx.algorithms.find_cliques(th))
        c = nx.algorithms.make_max_clique_graph(th)
        c = nx.relabel_nodes(c, {k: "".join(sorted(v)) for k, v in enumerate(cliques)})

        edges = c.edges()
        for src, dst in edges:
            c[src][dst]['weight'] = sum([1 if s in dst else 0 for s in src])

        return c

    def _extract_clique_tree(self, c: nx.Graph) -> nx.Graph:
        return nx.algorithms.maximum_spanning_tree(c)
    ...
    def _get_clique_tree(self) -> nx.Graph:
        ...
        # 3: create clique graph c - find maximal cliques
        # see https://networkx.org/documentation/stable/reference/algorithms/clique.html
        c = self._create_clique_graph(th)

        # 4: create clique tree from clique graph c - find Maximum Weight Spanning Tree in c
        ...
        if self.debug:
            ...
            nx.draw_networkx(g, ax=axs[0,0], with_labels=True)
            nx.draw_networkx(h, ax=axs[0,1], with_labels=True)
            nx.draw_networkx(th, ax=axs[1,0], with_labels=True)
            nx.draw_networkx(c, ax=axs[1,1], with_labels=True)
            nx.draw_networkx(t, ax=axs[1,2], with_labels=True)
            plt.show()
    ...
    def _load_factors(self) -> None:
        ...
        """
        factors = [n.to_factor() for n in self.bn.nodes.values()]

        for node in self.clique_tree.nodes:
            data = []
            for factor in factors:
                if all([name in node for name in factor.vars]):
                    data.append(factor)
            for factor in data:
                factors.remove(factor)
            self.clique_tree.nodes[node]['data'] = None if not data else functools.reduce(multiply_factors,
                                                                            [f.table for f in data[1:]],
                                                                            data[0].table)
            if self.debug:
                print(node)
                print([f.vars for f in data])
                print("\nX\n".join([str(f.table) for f in data]))
                print("=")
                print(self.clique_tree.nodes[node]['data'])
                print("-------------------------------------------")

    def _get_junction_tree(self, root_name: str = None) -> Tuple[nx.DiGraph, str]:
        """
    ...
    def _incorporate_evidence(self, jt: nx.DiGraph, evidence: Dict[str, int]) -> nx.DiGraph:
        ...
        for var in evidence:
            for node in jt.nodes():
                if jt.nodes[node]['data'] is not None and var in jt.nodes[node]['data'].index.names:
                    f_vars = jt.nodes[node]['data'].index.names
                    jt.nodes[node]['data'].reset_index(inplace=True)
                    jt.nodes[node]['data']['prob'].values[jt.nodes[node]['data'][var] == 1 - int(evidence[var])] = 0 # this 1 (:
                    jt.nodes[node]['data'].set_index(f_vars, inplace=True)
                    break

        return jt

    def _run_belief_propagation(self, uncalibrated_jt: nx.DiGraph, root_name: str) -> nx.DiGraph:
        """
        ...
        """
        def difference(src: str, dst: str) -> List[str]:
            return [s for s in src if s not in dst]

        def collect_dfs(node: str) -> None:
            neighbors = uncalibrated_jt.neighbors(node)
            if not neighbors:
                return
            ...
            for n in neighbors:
                ...
                if uncalibrated_jt.nodes[node]['data'] is not None and uncalibrated_jt.nodes[n]['data'] is not None:
                    vars = difference(n, node)
                    miu = sumout(uncalibrated_jt.nodes[n]['data'], vars)
                    uncalibrated_jt.nodes[node][n] = 1 / miu
                    uncalibrated_jt.nodes[node]['data'] = multiply_factors(uncalibrated_jt.nodes[node]['data'], miu)
                    if self.debug:
                        print(f'Collect: miu_{n}->{node} will be the sumout for {vars}')

        def distribute_dfs(node: str) -> None:
            neighbors = uncalibrated_jt.neighbors(node)
            if not neighbors:
                return
            ...
            for n in neighbors:
                if uncalibrated_jt.nodes[node]['data'] is not None and uncalibrated_jt.nodes[n]['data'] is not None:
                    vars = difference(node, n)
                    miu = sumout(uncalibrated_jt.nodes[node]['data'], vars)
                    miu = multiply_factors(miu, uncalibrated_jt.nodes[node][n])
                    uncalibrated_jt.nodes[n]['data'] = multiply_factors(uncalibrated_jt.nodes[n]['data'], miu)
                    if self.debug:
                        print(f'Distribute: miu_{node}->{n} will be the sumout for {vars}')

                distribute_dfs(n)


        collect_dfs(root_name)
        distribute_dfs(root_name)

        return uncalibrated_jt
    ...
    def _eval_query(self, calibrated_jt: nx.DiGraph, query: Dict[str, int]) -> float:
        ...
        for node in calibrated_jt.nodes:
            if all([q in node for q in query]) and calibrated_jt.nodes[node]['data'] is not None:
                vars = list(set(node).difference(list(query.keys())))
                p = sumout(calibrated_jt.nodes[node]['data'], vars)
                normalize(p)
                p.reset_index(inplace=True)
                for key in query:
                    p = p[p[key] == int(query[key])]
                return p['prob'].sum()
        return 0
    ...
    def run_query(self, query: Dict[str, int], evidence: Dict[str, int]) -> float:
        ...
        jt, root_name = self._get_junction_tree(root_name=root_name)

        # incorporate evidence
        uncalibrared_jt = self._incorporate_evidence(jt, evidence)

        # calibrate tree
        calibrated_jt = self._run_belief_propagation(uncalibrared_jt, root_name)
...
if __name__ == "__main__":
    ...
    jt = JunctionTree(bn=bn, debug=DEBUG)
